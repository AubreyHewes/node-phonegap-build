...
module.exports = function(options, callback) {
    ...
    zip.compress(paths.www, paths.build, function(e, filename) {
        ...
        config.local.load(function(e, data) {
            ...
            // optionally unlock keys if set within data (useful for automated unlocking during build)
            if (options.keys) {
                doUnlockKeysAndBuild(data, url, options, filename, callback);
            } else {
                doBuild(data, url, options, filename, callback);
            }
...
function doUnlockKeysAndBuild(data, url, options, filename, callback) {
	options.emitter.emit('log', 'unlocking keys...');

	function unlockKey(platform, params) {
		return when.promise(function (resolve, reject, notify) {
			console.log('Unlocking: /keys/' + platform + '/' + params.id);
			options.api.put('/keys/' + platform + '/' + params.id, params, function(e, response) {
				require('fs').writeFile('unlockkey-' + platform +'-'+params.id +'.json', JSON.stringify(response));
				if (e) {
					reject(e);
				}
				resolve(response);
	...
	Object.keys(options.keys).forEach(function(platform) {
		keysUnlocked = unlockKey(platform, options.keys[platform]);
	});

	when.all(keysUnlocked).then(function () {
		console.log('unlocked all keys');
		//doBuild(data, url, options, filename, callback);
	}, function (err) {
		console.log('failed to unlock all keys');
		console.log(err);
	});

}

function doBuild(data, url, options, filename, callback) {
...
module.exports.waitForComplete = function(options, callback) {
    ...
    options.api.get(url, function(e, data) {
        ...
        if (status === 'error') {
            callback(new Error('error occured while building the ' + platform + ' app: ' + data.error[platform]));
        }
